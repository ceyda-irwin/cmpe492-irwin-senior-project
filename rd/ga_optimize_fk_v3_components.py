...
print("Target CC features:", target_cc)
...
def fitness(candidate: np.ndarray) -> float:
    ...
    count_penalty = abs(candidate_cc["count"] - target_cc["count"]) / max(target_cc["count"], 1)
    mean_area_penalty = abs(candidate_cc["mean_area"] - target_cc["mean_area"]) / max(target_cc["mean_area"], 1e-8)
    max_area_penalty = abs(candidate_cc["max_area"] - target_cc["max_area"]) / max(target_cc["max_area"], 1e-8)
    ...
    total_error = (
        ...
        + 2.0 * radial_penalty
        + 2.0 * count_penalty
        + 1.2 * mean_area_penalty
        + 1.2 * max_area_penalty
    )
