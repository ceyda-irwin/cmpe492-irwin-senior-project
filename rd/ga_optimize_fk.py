...
GA_BEST_GEN_DIR = GA_OUT / "best_gen"
GA_BEST_GEN_DIR.mkdir(parents=True, exist_ok=True)
...
def save_best_image(best_result, generation):
    ...
    plt.axis("off")
    plt.tight_layout()
    plt.savefig(GA_BEST_GEN_DIR / f"best_gen_{generation:02d}.png", dpi=180, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    plt.tight_layout()
    plt.savefig(GA_OUT / "ga_fitness_progress.png", dpi=180)
    plt.show()
